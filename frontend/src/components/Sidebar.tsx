...
import { LayoutDashboard, FileText, Send, Package, FileCheck, Users, Activity, User as UserIcon, Building2 } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  // For specialist-only users, show limited navigation
  ...
  // Full navigation for all other roles
  const fullNavItems = [
    ...
    { path: '/specialists', label: 'Users', icon: Users, roles: ['Provider Admin', 'Supplier Representative'] },
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <Badge variant="secondary" className="text-xs px-1.5 py-0 mt-1">
              {currentUser.role === 'Provider Admin' ? 'Admin' : 
               currentUser.role === 'Supplier Representative' ? 'Supplier' :
               currentUser.role === 'Contract Coordinator' ? 'Coordinator' :
               'Specialist'}
            </Badge>
          </div>
        </div>
